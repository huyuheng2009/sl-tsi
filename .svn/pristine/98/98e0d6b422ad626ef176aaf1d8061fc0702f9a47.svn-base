...
import com.yogapay.couriertsi.dbutils.HumpBeanProcessor;
import com.yogapay.couriertsi.dbutils.HumpMatcher;
...
public class Dao {
	 ...
	 * @return 查询结果
	 */
	public <T> Page<T> find(Class<T> entityClass,String sql, Object[] params,
			PageRequest pageRequest) {
		queryRunner = new QueryRunner(dataSource);
		List<T> list = new ArrayList<T>();
		Long count = count(sql, params);
	...
	public long count(String sql, Object[] params) {
		...
		Long retVal = 0L;
		int index;
		index = (index = sql.indexOf("from"))==-1?sql.indexOf("FROM"):index;
		try {
			if (sql.contains("group by")) {
			sql = "select count(*) as count from (select count(*) as count1 " + sql.substring(index)+") tab";	
		}else {
			sql = "select count(*) as count " + sql.substring(index);	
		}
		//	sql = "select count(*) as count " + sql.substring(index);
	...
	// 查询 表Sequence 下一个值
	public Long nextval(String seqName) throws SQLException{
		String sql = "SELECT nextval('"+seqName+"')";
		Long seq = (Long) this.findBy(sql, 1);
		return seq;
	}

	public Log4jdbcProxyDataSource getDataSource() {
		return dataSource;
	}
