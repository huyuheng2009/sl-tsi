...
public class OrderService {
	...
	public void paySettleUpdate(String pay_serno) throws SQLException {
		String sql = "update order_info o left join courier_pay c on o.id=c.order_id "
				    +" set o.freight_settle_status=IF(c.freight>0,1,o.freight_settle_status),o.freight_settle_time=IF(c.freight>0,now(),o.freight_settle_time),"
				    +" o.behalf_settle_status=IF(c.behalf>0,1,o.behalf_pay_status),o.behalf_settle_time=IF(c.behalf>0,now(),o.behalf_settle_time) where c.pay_serno=? ";
		List<Object> list = new ArrayList<Object>();
        list.add(pay_serno) ;
		dao.update(sql,list.toArray()) ;
	}
