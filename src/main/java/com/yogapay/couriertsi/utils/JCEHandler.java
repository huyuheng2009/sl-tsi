...
public class JCEHandler {
	 ...
	 * @throws org.jpos.security.jceadapter.JCEHandlerException
	 */
	protected static byte[] extractDESKeyMaterial(short keyLength, Key clearDESKey)
    ...
    public static String generateDESKey(short keyLength)
            throws JCEHandlerException
    {
        ...
        try {
            ...
            if (keyLength > 64)
                k1 = KeyGenerator.getInstance("DESede", provider.getName());
            else {
                k1 = KeyGenerator.getInstance("DES", provider.getName());
            }
            generatedClearKey = k1.generateKey();
            clearKeyBytes = extractDESKeyMaterial(keyLength, generatedClearKey);
            Util.adjustDESParity(clearKeyBytes);
        } catch (Exception e) {
        ...
        return ISOUtil.hexString(clearKeyBytes);
    }
